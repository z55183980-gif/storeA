...
try {
    ...
    if (!in_array('currency',$columns,true)) $db->exec("ALTER TABLE products ADD COLUMN currency TEXT NOT NULL DEFAULT 'USD' CHECK(currency IN ('USD','CNY'))");
    ...
    if (!in_array('currency',$orderColumns,true)) $db->exec("ALTER TABLE orders ADD COLUMN currency TEXT NOT NULL DEFAULT 'USD' CHECK(currency IN ('USD','CNY'))");
...
try {
    ...
    if ($method === 'POST' && $route === 'orders') {
        ...
        $db->beginTransaction(); $resolved=[]; $total=0.0; $currency=null;
        foreach($items as $item){$stmt=$db->prepare("SELECT * FROM products WHERE id=? AND status='on_sale'");$stmt->execute([(int)($item['product_id']??0)]);$product=$stmt->fetch();$qty=max(1,min(99,(int)($item['quantity']??1)));if(!$product||$product['stock']<$qty)throw new RuntimeException('商品不存在或库存不足');$productCurrency=($product['currency']??'USD')==='CNY'?'CNY':'USD';if($currency!==null&&$currency!==$productCurrency){$db->rollBack();respond(['error'=>'不同币种商品不能合并结算'],422);}$currency=$productCurrency;$resolved[]=[$product,$qty];$total+=(float)$product['price']*$qty;}
        ...
        $db->commit(); respond(['id'=>$orderId,'order_no'=>$orderNo,'guest_token'=>$guest,'total_amount'=>number_format($total,2,'.',''),'currency'=>$currency??'USD','status'=>'pending','payment_status'=>'unpaid','payment'=>$paymentPublic],201);
    }
    ...
    if ($method==='GET' && $route==='admin/stats') {
        respond(['products'=>(int)$db->query("SELECT COUNT(*) FROM products WHERE status='on_sale'")->fetchColumn(),'orders'=>(int)$db->query('SELECT COUNT(*) FROM orders')->fetchColumn(),'revenue_usd'=>(float)$db->query("SELECT COALESCE(SUM(total_amount),0) FROM orders WHERE status NOT IN ('cancelled','refunded') AND currency='USD'")->fetchColumn(),'revenue_cny'=>(float)$db->query("SELECT COALESCE(SUM(total_amount),0) FROM orders WHERE status NOT IN ('cancelled','refunded') AND currency='CNY'")->fetchColumn(),'users'=>(int)$db->query('SELECT COUNT(*) FROM users')->fetchColumn(),'pending'=>(int)$db->query("SELECT COUNT(*) FROM orders WHERE status='pending'")->fetchColumn()]);
    }
    ...
    if ($method==='POST' && $route==='admin/products') {
        $name=requireText($data,'name',1,120);$price=(float)($data['price']??-1);$original=($data['original_price']??'')===''?null:(float)$data['original_price'];$currency=($data['currency']??'USD')==='CNY'?'CNY':'USD';$stock=(int)($data['stock']??999);$sales=(int)($data['sales']??0);if($price<0||($original!==null&&$original<0)||$stock<0||$sales<0)respond(['error'=>'price_stock_or_sales_invalid'],422);
        $stmt=$db->prepare('INSERT INTO products(name,description,price,original_price,currency,stock,sales,image_url,status,sort_order,category_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)');$stmt->execute([$name,mb_substr((string)($data['description']??''),0,5000),$price,$original,$currency,$stock,$sales,(string)($data['image_url']??''),($data['status']??'on_sale')==='off_sale'?'off_sale':'on_sale',(int)($data['sort_order']??0),(int)($data['category_id']??0)?:null]);respond(['id'=>(int)$db->lastInsertId()],201);
    }
    if (($method==='PUT' && preg_match('#^admin/products/(\d+)$#',$route,$match)) || ($method==='POST' && preg_match('#^admin/products/(\d+)/save$#',$route,$match))) {
        $original=($data['original_price']??'')===''?null:(float)$data['original_price'];$currency=($data['currency']??'USD')==='CNY'?'CNY':'USD';if($original!==null&&$original<0)respond(['error'=>'original_price_invalid'],422);$stmt=$db->prepare('UPDATE products SET name=?,description=?,price=?,original_price=?,currency=?,stock=?,sales=?,image_url=?,status=?,sort_order=?,category_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?');$stmt->execute([requireText($data,'name',1,120),mb_substr((string)($data['description']??''),0,5000),(float)($data['price']??0),$original,$currency,max(0,(int)($data['stock']??0)),max(0,(int)($data['sales']??0)),(string)($data['image_url']??''),($data['status']??'on_sale')==='off_sale'?'off_sale':'on_sale',(int)($data['sort_order']??0),(int)($data['category_id']??0)?:null,(int)$match[1]]);respond(['ok'=>true]);
    }
